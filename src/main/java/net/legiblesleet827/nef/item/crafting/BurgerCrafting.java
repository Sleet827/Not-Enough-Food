package net.legiblesleet827.nef.item.crafting;
...
public class BurgerCrafting extends CustomRecipe {
    ...
    public boolean matches(CraftingContainer container, Level level) {
        ...
        for (int i = 0; i < container.getContainerSize(); ++i) {
            ...
            if (!itemstack.isEmpty()) {
                if (itemstack.is(Registration.BURGER_BUN.get()) && !flag) {
                    if (itemstack.hasTag()) {
                    ...
                    try {
                        ...
                        ItemStack itemBelow = container.getItem(i + 3);
                        if (itemAbove.is(Registration.BURGER_BUN.get()) && itemBelow.is(Registration.BURGER_BUN.get())) {
                            flag1 = true;
                        }
                    } catch (Exception e) {
                        NotEnoughFood.LOGGER.error(e);
                    }
                } else if (itemstack.is(Registration.BURGER_BUN.get()) && !flag2) {
                    if (itemstack.hasTag()) {
    ...
    public ItemStack assemble(CraftingContainer container) {
        return new ItemStack(Registration.BURGER.get());
    }
    ...
    public RecipeSerializer<?> getSerializer() {
        return Registration.BURGER_CRAFTING.get();
    }
}
